from untitled import DataPreparation
from Modelsss import Models
from ParallelUtils import DataPreparation as dp
import numpy as np
...
X_train, Y_train, X_test, Y_test, description = dp.import_train_test(7, 21, 10000,
...
X_trains = DataPreparation.put_as_single_series(X_train)
Y_trains = DataPreparation.put_as_single_series_(Y_train)
X_tests = DataPreparation.put_as_single_series(X_test)
Y_tests = DataPreparation.put_as_single_series_(Y_test)
print(X_trains[1].shape)
extraction_time = time.time() - extraction_time

for i in range(0, len(X_trains)):
	Models.single_lstm_layer(X_trains[i], Y_trains[i], X_tests[i], Y_tests[i])
