...
def import_data(input_length, target_distance, number_of_samples, top_required_position=200):
    # This function returns a dataset including a number of evolutions for a song in the national charts among all the
    ...
    for n in range(0, number_of_samples):
        ...
        starting_date = dt.datetime.strptime(min_date, '%Y-%m-%d').date() + dt.timedelta(np.random.randint(max_offset))
        ...
        song_number = np.random.randint(0, len(daily_songs))
...
def main():
    seq_length = 3
    ss, countries_number = import_data(seq_length,1,3,50)
    t, tt = holdout_split(ss)
    mod = build_model(countries_number, seq_length)
    run_network(mod, split_sets(t, tt))


def build_model(sequence_size, sequence_length=None):

    dropout_perc = 0.2

    model = Sequential()
    layers = [sequence_size, 100, 100, sequence_size]

    model.add(LSTM(layers[1], input_shape=(sequence_length, sequence_size), return_sequences=True))
